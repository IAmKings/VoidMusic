...
- [x] 覆盖中断、重复导入、共享删除、缺失文件和孤儿恢复测试（仪器测试已编译，待真机连接后执行）。
...
阶段 1–3 的代码与测试已完成本地质量检查；阶段 0 的 WavDecoder 基线已由严格契约测试取代。下一批进入阶段 4，先补 DrumEngine 切换失败基线，再实现后台准备与无损切换；仍不改实时识别参数。
