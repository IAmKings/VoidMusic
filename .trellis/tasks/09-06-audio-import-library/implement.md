...
- [x] 增加 Schema、DAO、设置迁移与路径约束测试（JVM 已通过；Room 仪器测试已编译，待测试包真机窗口执行）。

质量门：旧设置准确迁移；新库可创建且约束生效；没有启动写入内置音色。

## 阶段 2 — WAV 校验与规范化（P0）

- [ ] 把 WAV 解析、校验和解码合并为单一边界模型。
- [ ] 覆盖 RIFF/WAVE、fmt/data、未知 chunk、奇数 padding、边界、帧对齐和 PCM16 单/双声道。
- [ ] 流式复制时执行 10 MiB 限制与 SHA-256。
- [ ] 实现双声道转单声道、真实重采样和规范化 PCM16 WAV 输出。
- [ ] 强制 5 秒时长上限并清理所有失败 staging 文件。
- [ ] 增加解析模糊输入、边界和规范化输出回读测试。

质量门：无越界、OOM 或残留临时文件；规范化文件可由同一解析器重新验证。

## 阶段 3 — 原子导入与一致性（P0）

- [ ] 实现 `copyKit`、`replacePad`、内容去重和事务提交。
- [ ] 增加数据库/移动/删除失败补偿和稳定错误类型。
- [ ] 实现共享资产引用检查、PENDING_DELETE 与轻量 `reconcile()`。
- [ ] 覆盖中断、重复导入、共享删除、缺失文件和孤儿恢复测试。

质量门：成功结果文件与数据库同时可用；所有失败路径保留原映射且可恢复。

## 阶段 4 — 音频准备与无损切换（P0）

- [ ] 用明确的内置/导入来源模型替换 `SampleRef` 布尔分支。
- [ ] 后台构建 `PreparedKit`，移除 MainScreen/主线程同步文件 I/O。
- [ ] DrumEngine 仅在准备成功后切换，并在启动失败时恢复旧音色。
- [ ] SoundPool 支持受控文件路径，Oboe 继续消费预解码 PCM。
- [ ] 回归连续击打、路由恢复、后台恢复和 STEP Transport。
...
阶段 1 的代码与测试已完成首轮质量检查；阶段 0 尚需在进入音频切换前补齐 WavDecoder 与 DrumEngine 失败基线。下一批进入阶段 2 的 WAV 严格解析、校验与规范化，不改实时识别参数与 JNI 混音逻辑。
